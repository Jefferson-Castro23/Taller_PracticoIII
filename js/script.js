...
function validar(nombre, dui, nit, year, placa, fallas){
  ...
  let expPlaca = /^[a-zA-Z]{1}[0-9]{3}[a-zA-Z]{1}[0-9]{2}|[a-zA-Z]{1}[0-9]{6}$/;
  ...
  if (!expDui.test(dui) || dui == ""){
    alert('ERROR: DUI inválido. Recuerde colocar el guion');
    return false;
  }
  ...
  if (!expPlaca.test(placa) || placa == ""){
    alert('ERROR: Placa inválida. Recuerde no colocar espacios');
    return false;
  }
...
function mostrar(){
  let tabla = document.getElementById('datos');
    tabla.innerHTML = "<table id='tablaResul'><tr class='filaTit'><td>Nombre</td><td>DUI</td><td>NIT</td><td>Marca</td><td>Modelo</td><td>Año</td><td>Color</td><td>Placa</td><td>Fallas</td><td>Fecha Ingreso</td></tr>";
